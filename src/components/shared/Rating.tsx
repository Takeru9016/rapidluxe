  ...
  return (
    ...
    >
      ...
      <path
        ...
        stroke={type === 'empty' ? 'var(--color-navy-border)' : 'var(--color-gold)'}
        strokeWidth={1.5}
        strokeLinecap="round"
...
export function Rating({
  ...
  return (
    <div className={cn('inline-flex items-center gap-1.5', className)}>
      ...
      <span className={cn('font-mono font-semibold text-[var(--color-gold)]', textSize)}>
        {rating.toFixed(1)}
      </span>
      {showCount && reviewCount !== undefined && (
        <span className={cn('font-sans text-[var(--color-text-secondary)]', textSize)}>
          ({reviewCount} reviews)
        </span>
      )}
    </div>
