...
export function EmptyState({ icon: Icon = PackageSearch, title, description, action, className }: EmptyStateProps) {
  return (
    <div className={cn('flex flex-col items-center justify-center text-center py-20 px-4', className)}>
      <div className="bg-[var(--color-gold)]/10 rounded-full p-4">
        <Icon className="w-12 h-12 text-[var(--color-gold)]" />
      </div>

      <h3 className="font-serif text-2xl text-white mt-6">{title}</h3>

      {description && (
        <p className="font-sans text-[var(--color-white-muted)] text-base mt-2 max-w-sm">
          {description}
        </p>
      )}
