...
export function CountdownTimer({
  ...
  if (variant === "inline") {
    if (expired) {
      return (
        <span
          className={cn(
            "font-sans text-sm text-(--color-text-secondary)",
            className,
          )}
        >
          Expired
    ...
    return (
      <span className={cn("font-mono text-sm text-(--color-coral)", className)}>
  ...
  if (expired) {
    return (
      <span
        className={cn(
          "inline-flex items-center px-3 py-1 rounded-full bg-white/5 border border-white/10 font-sans text-sm text-(--color-text-secondary)",
          className,
        )}
      >
        Deal Ended
