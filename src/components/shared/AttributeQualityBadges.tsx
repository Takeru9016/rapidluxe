...
export function AttributeQualityBadges({
  attributes,
  className,
}: AttributeQualityBadgesProps) {
  if (!attributes || attributes.length === 0) return null;

  return (
    <div
      className={cn(
        "flex flex-wrap gap-x-6 gap-y-3 py-4 border-t border-b border-(--color-navy-border)",
        className,
      )}
    >
      {attributes.map((attr) => (
        <div key={attr.label} className="flex flex-col items-start gap-1">
          ...
          <span
            className={cn(
              "px-2 py-0.5 rounded-full text-xs font-sans",
              badgeStyles[attr.quality],
            )}
          >
            {attr.quality}
          </span>
