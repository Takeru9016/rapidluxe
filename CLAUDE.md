...
Active Task:    none
Last Completed: Real company contact details rollout — replaced placeholder company/contact info with real Rapidluxe Pvt. Ltd. details (address, +91 91374 56611, info@rapidluxe.com, bookings@rapidluxe.com, GSTIN 27AAPCR1322N1Z, PAN AAPCR1322N) in InvoiceDocument.tsx (header+footer, now reads PAN like it already did GSTIN), ContactPageClient.tsx (CONTACT_BLOCKS), admin Settings page DEFAULT_VALUES, and newsletter route's stale "hello@rapidluxe.com" FROM fallback. Added GSTIN/PAN/NEXT_PUBLIC_SUPPORT_PHONE/NEXT_PUBLIC_SUPPORT_EMAIL to .env.example and .env.local (GSTIN/PAN were previously unset, so invoices were silently rendering "XXXXXXXXXXXX"). about/page.tsx and Footer.tsx had no hardcoded contact info (Sanity-driven / settings-API-driven) — nothing to change. Email templates in src/emails/ already use bookings@rapidluxe.com correctly via env fallback, no street address in any footer — left as-is. Known pre-existing bug, not fixed (out of scope): admin Settings page's main company-details form (companyName/gstin/contactEmail/whatsappNumber) onSubmit is `console.log` only — never persists, same dead-mock pattern as the old Deals page. npx tsc --noEmit: 0 errors.
```
